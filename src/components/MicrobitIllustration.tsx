export function MicrobitIllustration({ type }: { type: 'buttons' | 'led' }) {
  ...
  return (
    <svg className="microbit-art" viewBox="0 0 340 210" role="img" aria-label={type === 'buttons' ? 'Buttons A and B on a micro:bit' : 'A heart on the 5 by 5 LED display'}>
      ...
      {type === 'buttons' && <><path d="M62 71v-20M278 71v-20" stroke="#ffcf4a" strokeWidth="4" /><text x="170" y="184" textAnchor="middle" fill="#dbe7f0" fontSize="14">Press to trigger an event</text></>}
    </svg>
  )
}
