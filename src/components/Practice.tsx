...
export function Practice({ brief, checklist, starterCode, onBack, onSubmit }: { brief: string; checklist: string[]; starterCode: string; onBack: () => void; onSubmit: () => void }) {
  ...
  const loadStarter = () => {
    const editor = frame.current?.contentWindow
    if (!editor) return
    editor.postMessage({ type: 'pxteditor', id: crypto.randomUUID(), action: 'importproject', response: true, project: { text: { 'main.ts': starterCode, 'README.md': 'Buttons A/B practice', 'pxt.json': JSON.stringify({ name: 'Buttons and LED', dependencies: { core: '*' }, files: ['main.ts', 'README.md'] }) } } }, '*')
