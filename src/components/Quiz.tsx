...
function Match({ question, answer, setAnswer }: { question: MatchQuestion; answer?: Answer; setAnswer: (a: Answer) => void }) {
  ...
  return <div className="match-grid">{question.blocks.map((block) => <div className="match-row" key={block.id}><BlockVisual block={block} compact /><label><span>Ghép với</span><select aria-label={`Chức năng của ${block.label}`} value={value[block.id] ?? ''} onChange={(e) => setAnswer({ ...value, [block.id]: e.target.value })}><option value="">Chọn chức năng…</option>{question.functions.map((f) => <option key={f.id} value={f.id}>{f.text}</option>)}</select></label></div>)}</div>
}
...
function Order({ question, answer, setAnswer }: { question: OrderQuestion; answer?: Answer; setAnswer: (a: Answer) => void }) {
  ...
  return <div className="order-list">{initial.map((id, index) => { const block = question.items.find((item) => item.id === id)!; return <div className={`order-item ${selected === id ? 'selected' : ''}`} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', id)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); move(initial.indexOf(e.dataTransfer.getData('text/plain')), index) }} key={id}>
    <button className="order-position" aria-label={`Đặt khối đã chọn vào vị trí ${index + 1}`} onClick={() => selected ? place(index) : setSelected(id)}>{index + 1}</button><BlockVisual block={block} compact /><div className="move-buttons"><button aria-label="Đưa lên" disabled={index === 0} onClick={() => move(index, index - 1)}>↑</button><button aria-label="Đưa xuống" disabled={index === initial.length - 1} onClick={() => move(index, index + 1)}>↓</button></div>
  </div>})}<p className="interaction-hint">Kéo thả, dùng ↑ ↓, hoặc chọn khối rồi chọn số vị trí.</p></div>
}
...
function Fill({ question, answer, setAnswer }: { question: FillQuestion; answer?: Answer; setAnswer: (a: Answer) => void }) {
  ...
  return <div className="fill-layout"><div className="program-stack"><BlockVisual block={{ id: 'event-b', label: 'khi nút B được nhấn', kind: 'input', icon: 'B' }} /><button className={`drop-zone ${placed ? 'filled' : ''}`} aria-label="Vị trí đặt khối còn thiếu" onClick={() => selected && drop(selected)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); drop(e.dataTransfer.getData('text/plain')) }}>{placed ? <BlockVisual block={question.choices.find((item) => item.id === placed)!} compact /> : <><span>+</span> {selected ? 'Chạm để đặt khối đã chọn' : 'Thả hoặc chọn khối rồi chạm vào đây'}</>}</button></div><div className="block-bank" aria-label="Kho khối">{question.choices.map((block) => <button key={block.id} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', block.id)} onClick={() => setSelected(block.id)} className={selected === block.id ? 'selected' : ''} aria-pressed={selected === block.id}><BlockVisual block={block} compact /></button>)}</div></div>
}
...
export function Quiz({ lesson, answers, setAnswer, onNext }: Props) {
  ...
  return <section className="page"><div className="section-heading left"><span className="kicker">Nhiệm vụ kiến thức</span><h1>Giải mã các khối lệnh</h1><p>Hoàn thành đủ 6 câu rồi sang phòng thực hành. Câu bỏ trống nhận 0 điểm.</p></div><div className="quiz-status">Đã tương tác <b>{answered}/{lesson.questions.length}</b> câu</div>
    <div className="questions">{lesson.questions.map((question, index) => <article className="question-card" key={question.id} data-question={question.id}><div className="question-title"><span>{String(index + 1).padStart(2, '0')}</span><div><p>{question.type === 'choice' ? 'Trắc nghiệm' : question.type === 'match' ? 'Ghép cặp' : question.type === 'order' ? 'Sắp xếp' : 'Điền khối' } · {question.weight} điểm</p><h2>{question.prompt}</h2></div></div>
    ...
    </article>)}</div><div className="actions between"><span>Em vẫn có thể quay lại sửa trước khi nộp.</span><button className="primary" onClick={onNext}>Đến phòng thực hành <span>→</span></button></div>
  </section>
}
